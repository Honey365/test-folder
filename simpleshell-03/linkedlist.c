...
alias_t *add_alias_end(alias_t **head, character *name, character *value);
void free_alias_list(alias_t *head);
list_t *add_node_end(list_t **head, character *dir);
...
alias_t *add_alias_end(alias_t **head, character *name, character *value)
{
	...
	new_node->next = NULL;
	new_node->name = malloc(sizeof(character) * (_strlen(name) + 1));
	if (!new_node->name)
	{
		free(new_node);
		return (NULL);
